...
defineLazyP5Property('AABB', boundConstructorFactory(AABB));
  ...
  function OBB(pInst, width, height, center, rotation) {
    ...
    /**
     * Get a list of unique possible separating axes between this shape and another.
     * @param {OBB} other
     * @returns {Array.<number>} unique list of angles representing possible separating axes
     */
     ...
     * @return {number}
     */
    this.getRadiusOnAxis = function (axis) {
      ...
      // Should be slightly faster than Vector.fromAngle because it requires no trig.
      var thisHalfHeightVector = new p5.Vector(thisHalfWidthVector.y, -thisHalfWidthVector.x).setMag(_height / 2);
    ...
    this.collide = function (other) {
      ...
      for (var i = 0; i < separatingAxes.length; i++) {
        ...
        // Get the radii of the two shapes along the current axis
        var thisRadiusAlongAxis = this.getRadiusOnAxis(axis);
        pInst.LOGGING && console.log('thisRadiusAlongAxis: ' + thisRadiusAlongAxis);
        var otherRadiusAlongAxis = other.getRadiusOnAxis(axis);
