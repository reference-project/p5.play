  ...
  this.AABBops = function(type, target, callback) {
    ...
    for(var i=0; i<others.length; i++)
      if(this !== others[i] && !this.removed) //you can check collisions within the same group but not on itself
      {
        ...
        {
        if(type === 'overlap') {
            var thisOBB = new OBB(pInst, this.width, this.height, this.position, radians(this.rotation));
            var otherOBB = new OBB(pInst, other.width, other.height, other.position, radians(other.rotation));
            displacement = thisOBB.collide(otherOBB);
...
defineLazyP5Property('AABB', boundConstructorFactory(AABB));
  ...
  /**
   * An [O]riented [B]ounding [B]ox.
   * @param width
   * @param height
   * @param {p5.Vector} center in world-space
   * @param {number} rotation about center, in radians
   * @constructor
   */
  function OBB(pInst, width, height, center, rotation) {
    // These are 'private' to this OBB
    var _width = width;
    var _height = height;
    var _center = center;
    var _rotation = rotation;

    // Define public getters and setters for the fundamental dimensions of this
    // box, which allow us to precompute certain values when they are set.
    Object.defineProperties(this, {
      'width': {
        ...
        set: function (w) {
          _width = w;
          // TODO: Recompute things we could precompute?
        }
      },
      'height': {
        enumerable: true,
    ...
    this.draw = function () {
      ...
      pInst.translate(_center.x, _center.y);
      if (pInst._angleMode === pInst.RADIANS) {
        pInst.rotate(radians(_rotation));
      } else {
        pInst.rotate(_rotation);
      }
      pInst.noFill();
      pInst.stroke('pink');
      pInst.strokeWeight(3);
      pInst.rect(0, 0, _width, _height);
      pInst.pop();
    };

    // Compute the smallest vector that would project this out of another collider.
    this.collide = function (other) {
      var differenceOfCenters = p5.Vector.sub(other.center, _center);

      //this.draw();
      //other.draw();

      // Separating axis theorem
      // For each separating axis (and we should deduplicate any parallels),
      // compute each shape's projection onto the line, and determine whether they
      // overlap.  If they do, store the size of the overlap - the smallest one
      // will be our displacement vector.  If one does not, the shapes are
      // not touching and we can early-out.
      var separatingAxes = [];
      var smallestOverlapAmount = Infinity;
      var smallestOverlapVector = null;

      // Push this shape's axes
      // For a rect, its separating axes are parallel to its sides
      separatingAxes.push(_rotation);
      separatingAxes.push(_rotation + Math.PI / 2);

      // Push other shape's axes
      // TODO: extract to helper so we can generalize
      separatingAxes.push(other.rotation);
      separatingAxes.push(other.rotation + Math.PI / 2);

      // Remove duplicate separating axes
      // TODO: Extract this
      separatingAxes.sort();
      var currentAxis = separatingAxes[separatingAxes.length - 1];
      for (var k = separatingAxes.length - 2; k >= 0; k--) {
        if (separatingAxes[k] === currentAxis) {
          separatingAxes.splice(k, 1);
        } else {
          currentAxis = separatingAxes[k];
        }
      }
      pInst.LOGGING && console.log("Axes to check: " + JSON.stringify(separatingAxes));

      var axis, directionAlongAxis, centerDistanceAlongAxis;
      for (var i = 0; i < separatingAxes.length; i++) {
        axis = p5.Vector.fromAngle(separatingAxes[i]);
        pInst.LOGGING && console.log('axis: ' + axis);
        // If the size of the vector between the centers projected onto the
        // separating axis is larger than the combined halfwidths of the shapes
        // projected onto the axis, the shapes do not overlap on this axis.
        directionAlongAxis = p5.Vector.project(differenceOfCenters, axis);
        centerDistanceAlongAxis = directionAlongAxis.mag();
        pInst.LOGGING && console.log('centerDistance: ' + centerDistanceAlongAxis);

        // How to project a rect onto the axis:
        // Project the center-corner vectors for two adjacent corners
        // onto the axis.  Use the larger magnitude.
        var thisHalfWidthVector = p5.Vector.fromAngle(_rotation).mult(_width / 2);
        pInst.LOGGING && console.log('thisHalfWidthVector: ' + thisHalfWidthVector);
        var thisHalfHeightVector = p5.Vector.fromAngle(_rotation + Math.PI / 2).mult(_height / 2);
        pInst.LOGGING && console.log('thisHalfHeightVector: ' + thisHalfHeightVector);
        var thisHalfDiagonal1 = p5.Vector.add(thisHalfWidthVector, thisHalfHeightVector);
        pInst.LOGGING && console.log('thisHalfDiagonal1: ' + thisHalfDiagonal1);
        var thisHalfDiagonal2 = p5.Vector.sub(thisHalfWidthVector, thisHalfHeightVector);
        pInst.LOGGING && console.log('thisHalfDiagonal2: ' + thisHalfDiagonal2);
        var thisRadiusAlongAxis = Math.max(p5.Vector.project(thisHalfDiagonal1, axis).mag(), p5.Vector.project(thisHalfDiagonal2, axis).mag());
        pInst.LOGGING && console.log('thisRadiusAlongAxis: ' + thisRadiusAlongAxis);

        // TODO: Generalize asking other for radius along axis
        var otherHalfWidthVector = p5.Vector.fromAngle(other.rotation).mult(other.width / 2);
        var otherHalfHeightVector = p5.Vector.fromAngle(other.rotation + Math.PI / 2).mult(other.height / 2);
        var otherHalfDiagonal1 = p5.Vector.add(otherHalfWidthVector, otherHalfHeightVector);
        var otherHalfDiagonal2 = p5.Vector.sub(otherHalfWidthVector, otherHalfHeightVector);
        var otherRadiusAlongAxis = Math.max(p5.Vector.project(otherHalfDiagonal1, axis).mag(), p5.Vector.project(otherHalfDiagonal2, axis).mag());
        pInst.LOGGING && console.log('otherRadiusAlongAxis: ' + otherRadiusAlongAxis);

        var overlapAmount = thisRadiusAlongAxis + otherRadiusAlongAxis - centerDistanceAlongAxis;
        pInst.LOGGING && console.log('overlapAmount: ' + overlapAmount);
        if (overlapAmount <= 0) {
          // These shapes are separated along this axis.  Return a displacement zero-vector.
          return new p5.Vector();
        } else if (overlapAmount < smallestOverlapAmount) {
          smallestOverlapAmount = overlapAmount;
          smallestOverlapVector = directionAlongAxis.setMag(-overlapAmount);
        }
      }

      // If we make it out of the loop, we overlap on all possible axes and we
      // have found the smallest overlap vector, which we should return.
      return smallestOverlapVector;
    };
  }
  defineLazyP5Property('OBB', boundConstructorFactory(OBB));
