...
defineLazyP5Property('AABB', boundConstructorFactory(AABB));
  ...
  function OBB(pInst, width, height, center, rotation) {
    ...
    var _potentialAxes = computePotentialSeparatingAxes();

    function computePotentialSeparatingAxes() {
      // For a rect, its separating axes are parallel to its sides
      return [
        _rotation, // Parallel to width
        _rotation + Math.PI / 2 // Parallel to height
      ];
    }
    ...
    Object.defineProperties(this, {
      ...
      'rotation': {
        ...
        set: function (r) {
          _rotation = r;
          _potentialAxes = computePotentialSeparatingAxes();
        }
      },
      'potentialAxes': {
        enumerable: true,
        get: function () {
          return _potentialAxes;
        }
      }
    });
    ...
    this.getPossibleAxesSeparatingThisFrom = function (other) {
      var possible = _potentialAxes.concat(other.potentialAxes);
      // Remove duplicate axes without reordering array (which might speed things up)
      return possible.filter(function (item, pos, self) {
    ...
    this.collide = function (other) {
      ...
      var separatingAxes = this.getPossibleAxesSeparatingThisFrom(other);
