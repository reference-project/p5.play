...
function Sprite(pInst, _x, _y, _w, _h) {
  ...
  */
  this.setCollider = function(type, offsetX, offsetY, width, height, rotation) {
    ...
    if (_type === 'point') {
      this.collider = new p5.CollisionPoint(center, offset);
    } else if (_type === 'circle') {
      this.collider = new p5.CollisionCircle(Math.floor(width / 2), center, offset);
    } else if (_type === 'aabb') {
      this.collider = new p5.AxisAlignedBoundingBox(width, height, center, offset);
    } else if (_type === 'obb') {
      if (rotation === undefined) {
        rotation = this.rotation;
      }
      this.collider = new p5.OrientedBoundingBox(width, height, center, radians(rotation), offset);
    }
  ...
  this.getBoundingBox = function() {
    ...
    return new p5.AxisAlignedBoundingBox(w, h, this.position);
  };
 ...
 */
p5.CollisionShape.prototype.draw = function(sketch) {
  sketch.push();
 ...
 * @param {p5.Vector} center
 */
p5.CollisionCircle = function(radius, center) {
  p5.CollisionShape.call(this, center);
 ...
 * @param {p5.Vector} center
 */
p5.AxisAlignedBoundingBox = function(width, height, center) {
  p5.CollisionShape.call(this, center);
 ...
 */
p5.OrientedBoundingBox = function(width, height, center, rotation) {
  p5.CollisionShape.call(this, center);
 ...
 * @return {Array.<p5.Vector>}
 */
p5.OrientedBoundingBox.prototype._computePotentialSeparatingAxes = function() {
 ...
 * @return {Array.<p5.Vector>}
 */
p5.OrientedBoundingBox.prototype._computeHalfDiagonals = function() {
...
p5.prototype._warn = function(message) {
  ...
  /**
   * 3x3 Matrix used for storing 2D transformations.
   * @class p5.Transform2D
   * @constructor
   * @param {p5.Transform2D|Float32Array|Array.<number>} [source]
   */
  p5.Transform2D = function(source) {
    ...
    if (source instanceof Float32Array || Array.isArray(source)) {
      this.mat3 = new Float32Array(source);
    } else if (source instanceof p5.Transform2D) {
      this.mat3 = new Float32Array(source.mat3);
    } else {
      this.mat3 = new Float32Array(
        [1, 0, 0,
         0, 1, 0]);
    }
  };
  ...
  p5.Transform2D.prototype.getTranslation = function() {
    return new p5.Vector(this.mat3[2], this.mat3[5]);
  };

  p5.Transform2D.prototype.getScale = function() {
    return new p5.Vector(
      new p5.Vector(this.mat3[0], this.mat3[3]).mag(),
      new p5.Vector(this.mat3[1], this.mat3[4]).mag()
    );
  };

  p5.Transform2D.prototype.toString = function() {
    var values = [];
    for (var i = 0; i < 6; i++) {
      values.push(this.mat3[i]);
    }
    return '[ ' + values.join(', ') + ' ]';
  };

  /**
   * Check whether two transforms or transform matrices have the same
   * components.
   * @method equals
  ...
  p5.Transform2D.prototype.equals = function(other) {
    var otherMat3;
    if (other instanceof p5.Transform2D) {
      otherMat3 = other.mat3;
    } else if (other instanceof Float32Array || Array.isArray(other)) {
      otherMat3 = other;
    } else {
   ...
   * @method mult
   * @static
   * @param {p5.Transform2D|Float32Array|Array.<number>} t1
   * @param {p5.Transform2D|Float32Array|Array.<number>} t2
   * @param {p5.Transform2D} [dest]
   * @returns {p5.Transform2D}
   */
  p5.Transform2D.mult = function(t1, t2, dest) {
    ...
    if (t1 instanceof p5.Transform2D) {
      t1_0 = t1.mat3[0];
      t1_1 = t1.mat3[1];
      t1_2 = t1.mat3[2];
      t1_3 = t1.mat3[3];
    ...
    if (t2 instanceof p5.Transform2D) {
      t2_0 = t2.mat3[0];
      t2_1 = t2.mat3[1];
      t2_2 = t2.mat3[2];
      t2_3 = t2.mat3[3];
    ...
    dest.mat3[0] = t1_0*t2_0 + t1_1*t2_3;
    dest.mat3[1] = t1_0*t2_1 + t1_1*t2_4;
    dest.mat3[2] = t1_0*t2_2 + t1_1*t2_5 + t1_2;

    dest.mat3[3] = t1_3*t2_0 + t1_4*t2_3;
    dest.mat3[4] = t1_3*t2_1 + t1_4*t2_4;
    dest.mat3[5] = t1_3*t2_2 + t1_4*t2_5 + t1_5;
  ...
  /**
   * Get a new transform representing the given translation.
   * @method translation
   * @static
   ...
   *   // 1. Two numbers
   *   var t1 = p5.Transform2D.translation(x, y)
   *   // 2. One vector
   *   var t2 = p5.Transform2D.translation(new p5.Vector(x, y));
   */
  p5.Transform2D.translation = function () {
    return p5.Transform2D.prototype.translate.apply(new p5.Transform2D(), arguments);
  };
  ...
  /**
   * Get a new transform representing the given scale operation
   * @method scaled
   * @static
   * @return {p5.Transform2D}
   * @example
   ...
   *   // 1. One scalar value
   *   var t1 = p5.Transform2D.scaled(uniformScale);
   *   // 1. Two scalar values
   *   var t2 = p5.Transform2D.scaled(scaleX, scaleY);
   *   // 2. One vector
   *   var t3 = p5.Transform2D.scaled(new p5.Vector(scaleX, scaleY));
   */
  p5.Transform2D.scaled = function () {
    return p5.Transform2D.prototype.scale.apply(new p5.Transform2D(), arguments);
  };
  ...
  p5.Vector.prototype.transform = function(transform) {
    ...
    var t = transform.mat3;
    var x = this.x;
    var y = this.y;
    this.x = t[0]*x + t[1]*y + t[2];
    this.y = t[3]*x + t[4]*y + t[5];
    return this;
  };
