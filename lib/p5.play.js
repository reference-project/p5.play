  ...
  this.AABBops = function(type, target, callback) {
    ...
    for(var i=0; i<others.length; i++)
      if(this !== others[i] && !this.removed) //you can check collisions within the same group but not on itself
      {
        ...
        {
        if(type === 'overlap') {
            var thisOBB = new OrientedBoundingBox(pInst, this.width, this.height, this.position, radians(this.rotation));
            var otherOBB = new OrientedBoundingBox(pInst, other.width, other.height, other.position, radians(other.rotation));
