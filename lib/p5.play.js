...
function Sprite(pInst, _x, _y, _w, _h) {
  ...
  */
  this.setCollider = function(type, offsetX, offsetY, width, height, rotation) {
    ...
    if (_type === 'point') {
      this.collider = p5.CollisionPoint.createFromSprite(this, offset);
    } else if (_type === 'circle') {
      this.collider = p5.CollisionCircle.createFromSprite(this, offset, width);
    } else if (_type === 'aabb') {
      this.collider = p5.AxisAlignedBoundingBox.createFromSprite(this, offset, width, height);
    } else if (_type === 'obb') {
      this.collider = p5.OrientedBoundingBox.createFromSprite(this, offset, width, height, radians(rotation));
    }
  ...
  */
  ...
  this._collideWith = function(type, target, callback) {
    ...
    if (target instanceof Sprite) {
      return this._collideWithSprite(type, target, callback);
      ...
      for(var i=0; i<others.length; i++) {
        result = this._collideWithSprite(type, others[i], callback) || result;
      }
      return result;
    }
   ...
   * @return {boolean} true if a collision occurred
   */
  this._collideWithSprite = function(type, target, callback) {
    ...
    if (target.collider === undefined) {
      target.setDefaultCollider();
    }

    if (!this.collider || !target.collider) {
    ...
    var displacement = this._findDisplacement(target);
    ...
    if (type === 'collide' || type === 'displace' || type === 'bounce') {
      if (type === 'displace' && !target.immovable) {
        target.position.sub(displacement);
      } else if ((type === 'collide' || type === 'bounce') && !this.immovable) {
        this.position.add(displacement);
        this.previousPosition = createVector(this.position.x, this.position.y);
        this.newPosition = createVector(this.position.x, this.position.y);
        // We modified this, let's update the collider to match
        this.collider.updateFromSprite(this);
      }
      ...
      if (type === 'bounce') {
        ...
        var totalMass = this.mass + target.mass;
        var m1 = 0, m2 = 0;
        if (this.immovable) {
          m2 = 2;
        } else if (target.immovable) {
          m1 = 2;
        } else {
          m1 = 2 * target.mass / totalMass;
          m2 = 2 * this.mass / totalMass;
        }
        var newVel1 = dx1.mult(m1 * p5.Vector.sub(this.velocity, target.velocity).dot(dx1) / magnitude);
        var newVel2 = dx2.mult(m2 * p5.Vector.sub(target.velocity, this.velocity).dot(dx2) / magnitude);

        this.velocity.sub(newVel1.mult(this.restitution));
        target.velocity.sub(newVel2.mult(target.restitution));
      }
    }
    ...
    if (typeof callback === 'function') {
      callback.call(this, this, target);
    }
    return true;
  };
  ...
  this._findDisplacement = function(target) {
    ...
    // velocity itself, we are close enough to have tunnelled this frame.
    ...
    if (squareVelocity > centerSquareDistanceOnVelocityAxis) {
      ...
      var thisDelta = p5.Vector.sub(thisOriginalPosition, this.previousPosition);
      var targetDelta = p5.Vector.sub(targetOriginalPosition, target.previousPosition);
      ...
      // Put an upper bound on the timestep (eight checks per frame) so we don't
      // destroy performance too much - this means very small fast things _will_
      // still tunnel, but not too often.
      var timestep = Math.max(0.125, radiusOnVelocityAxis / Math.sqrt(squareVelocity));
      for (var i = 0; i < 1; i += timestep) {
        // Check for displacement at the interpolated position
        this.position = p5.Vector.add(this.previousPosition, p5.Vector.mult(thisDelta, i));
        target.position = p5.Vector.add(target.previousPosition, p5.Vector.mult(targetDelta, i));
        ...
        if (displacement.x !== 0 || displacement.y !== 0) {
          // These sprites are overlapping - we have our displacement,
          // restore the sprites' postions and get out of here.
          //console.log('Overlap found at interpolation ' + i);
          return displacement;
        }
      }
...
p5.prototype._warn = function(message) {
  ...
  p5.CollisionShape.prototype.setParentTransform = function(parent) {
    if (parent instanceof Sprite) {
      this._parentTransform
        .clear()
        .scale(parent.scale) // TODO: ScaleX and ScaleY?
  ...
  p5.CollisionShape.prototype.getBoundingBox = function() {
    var radiusOnX = this._getRadiusOnAxis(p5.CollisionShape.WORLD_AXES[0]);
    var radiusOnY = this._getRadiusOnAxis(p5.CollisionShape.WORLD_AXES[1]);
    return {
   ...
   */
  p5.CollisionPoint.createFromSprite = function(sprite, offset) {
    // Create the collision shape at the transformed offset
    var shape = new p5.CollisionPoint(offset);
   ...
   */
  p5.CollisionCircle.createFromSprite = function(sprite, offset, radius) {
   ...
   * @see p5.CollisionShape.prototype.getsDimensionsFromSprite
   */
  p5.CollisionCircle.prototype.updateFromSprite = function(sprite) {
   ...
   * @protected
   */
  p5.CollisionCircle.prototype._onTransformChanged = function() {
   ...
   * @private
   */
  p5.CollisionCircle.prototype._computeScaledRadius = function() {
   ...
   * @return {p5.CollisionCircle}
   */
  p5.AxisAlignedBoundingBox.createFromSprite = function(sprite, offset, width, height) {
    var customSize = typeof width === 'number' && typeof height === 'number';
    var box = new p5.AxisAlignedBoundingBox(
      offset,
      customSize ? width : 1,
      customSize ? height : 1
    );
   ...
   * @protected
   */
  p5.AxisAlignedBoundingBox.prototype._onTransformChanged = function() {
    p5.CollisionShape.prototype._onTransformChanged.call(this);
    this._computeHalfDiagonals();
  };
   ...
   * @return {p5.CollisionCircle}
   */
  p5.OrientedBoundingBox.createFromSprite = function(sprite, offset, width, height, rotation) {
    var customSize = typeof width === 'number' && typeof height === 'number';
    var box = new p5.OrientedBoundingBox(
      offset,
      customSize ? width : 1,
      customSize ? height : 1,
      rotation
   ...
   * @see p5.CollisionShape.prototype.getsDimensionsFromSprite
   */
  p5.OrientedBoundingBox.prototype.updateFromSprite = function(sprite) {
    if (this.getsDimensionsFromSprite) {
      this._width = sprite.width;
      this._height = sprite.height;
    }
