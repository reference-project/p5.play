...
function Sprite(pInst, _x, _y, _w, _h) {
  ...
  */
  this.update = function() {
    ...
    if(!this.removed)
    {
      ...
      if (this.collider instanceof OrientedBoundingBox || this.collider instanceof BoundingCircle) {
  ...
  */
  this.setCollider = function(type, offsetX, offsetY, width, height) {
    if (!(type === 'rectangle' || type === 'circle' || type === 'obb' || type === 'bc')) {
      throw new TypeError('setCollider expects the first argument to be either "circle" or "rectangle" or "obb"');
    ...
    } else if (type === 'obb' && !(arguments.length === 1)) {
      throw new TypeError('Usage: setCollider("obb")');
    } else if (type === 'bc' && !(arguments.length === 1)) {
      throw new TypeError('Usage: setCollider("bc")');
    }
    ...
    if (type === 'rectangle' && arguments.length === 1) {
      ...
      this.collider = new OrientedBoundingBox(pInst, this.width, this.height, this.position, radians(this.rotation));
    } else if (type === 'bc') {
      this.collider = new BoundingCircle(pInst, Math.floor(Math.max(this.width, this.height) / 2), this.position);
    }
        ...
        if (this.collider === undefined || other.collider === undefined) {
          ...
          if (displacement.x === 0 && displacement.y === 0) {
            ...
            if (type === 'collide' || type === 'displace' || type === 'bounce') {
              ...
              if (type === 'bounce') {
                if ((this.collider instanceof OrientedBoundingBox || this.collider instanceof BoundingCircle) &&
                    (other.collider instanceof OrientedBoundingBox || other.collider instanceof BoundingCircle)) {
                  // New collision system: Use the displacement direction to determine velocity change
...
defineLazyP5Property('AABB', boundConstructorFactory(AABB));
   ...
   * @constructor
   */
  function OrientedBoundingBox(pInst, width, height, center, rotation) {
    // These are 'private' to this OrientedBoundingBox
    var _width = width;
    var _height = height;
    var _center = center;
    var _rotation = rotation;
    // Potential separating axes this shape contributes to a collision.
    var _potentialAxes = computePotentialSeparatingAxes();
    // This box's half-diagonals, for use computing its projected radius.
    var _halfDiagonals = computeHalfDiagonals();

    function computePotentialSeparatingAxes() {
      // For a rect, its sides define separating axes.
      var widthAligned = p5.Vector.fromAngle(_rotation);
    ...
    function computeHalfDiagonals() {
      ...
      // box changes.
      var halfWidth = p5.Vector.fromAngle(_rotation).mult(_width / 2);
      // Clockwise perpendicular rotation of <x, y> is <y, -x>
      // Should be faster than Vector.fromAngle because it requires no trig.
      var halfHeight = new p5.Vector(halfWidth.y, -halfWidth.x).setMag(_height / 2);
      return [
    ...
    Object.defineProperties(this, {
      'width': {
        ...
        set: function(w) {
          _width = w;
          _halfDiagonals = computeHalfDiagonals();
        }
      },
      'height': {
        enumerable: true,
     ...
     * @param {Sprite} sprite
     */
    this.updateFromSprite = function(sprite) {
      ...
      _center = sprite.position;
      _rotation = radians(sprite.rotation);
      if (_rotation !== oldRotation) {
        _potentialAxes = computePotentialSeparatingAxes();
        _halfDiagonals = computeHalfDiagonals();
      } else if (_width !== oldWidth || _height !== oldHeight) {
        _halfDiagonals = computeHalfDiagonals();
      }
    };
    ...
    this.draw = function() {
      pInst.push();
      pInst.rectMode(p5.prototype.CENTER);
      pInst.translate(_center.x, _center.y);
      if (pInst._angleMode === pInst.RADIANS) {
        pInst.rotate(_rotation);
      } else {
        pInst.rotate(degrees(_rotation));
      }
      pInst.noFill();
      pInst.stroke('pink');
      pInst.stroke(0, 255, 0);
     ...
     */
    this.collide = function(other) {
      return computeSmallestDisplacement(this, other);
    };
     ...
     */
    this.getCandidateAxes = function() {
     ...
     * @private (but may be used by other collision shapes)
     * @param {p5.Vector} axis
     * @return {number}
     */
    this._getRadiusOnAxis = function(axis) {
      ...
      return Math.max(
        p5.Vector.project(_halfDiagonals[0], axis).mag(),
        p5.Vector.project(_halfDiagonals[1], axis).mag());
    };
  }
  defineLazyP5Property('OrientedBoundingBox', boundConstructorFactory(OrientedBoundingBox));

  /**
   * Compute the smallest movement needed to move one collision shape out of
   * another.  If the shapes are not overlapping, returns a zero vector to
   * indicate that no displacement is necessary.
   * @param {OrientedBoundingBox} displacee
   * @param {OrientedBoundingBox} displacer
   * @returns {p5.Vector}
   */
  function computeSmallestDisplacement(displacee, displacer) {
    ...
    // We speed things up with an additional assumption that we will only
    // compare shapes that are symmetrical about their center point:
    // Circles, ellipses, and rectangles are OK.  Other convex shapes,
    // (triangles, pentagons) would need special handling.  This
    // lets us compare the shapes' radii to the distance between their
    // centers, even for non-circular shapes.
    ...
    // separating axis.
    var candidateAxes = deduplicateParallelVectors(displacee.getCandidateAxes(displacer).concat(displacer.getCandidateAxes(displacee)));
   ...
   * @constructor
   */
  function BoundingCircle(pInst, radius, center) {
    // These are 'public' and can safely be modified
    this.radius = radius;
    this.center = center;
    ...
    this.draw = function() {
      ...
      pInst.stroke(0, 255, 0);
      pInst.rectMode(p5.prototype.CENTER);
      pInst.ellipse(this.center.x, this.center.y, this.radius*2, this.radius*2);
      pInst.pop();
    };
     ...
     * @return {Array.<p5.Vector>}
     */
    this.getCandidateAxes = function(other) {
      ...
      if (other instanceof BoundingCircle) {
        // When checking against another circle we only need to check the axis
        // through both circles' centers.
        return [
          p5.Vector.sub(other.center, this.center)
        ];
      } else if (other instanceof OrientedBoundingBox) {
        // There are four possible separating axes with an OBB - one for each
     ...
     * @return {number}
     */
    this._getRadiusOnAxis = function() {
      // For a circle, this is _always_ just the radius.
      return this.radius;
    };
  }
  defineLazyP5Property('BoundingCircle', boundConstructorFactory(BoundingCircle));
