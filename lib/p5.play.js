  ...
  */
  this.update = function() {
    ...
    if(!this.removed)
    {
      ...
      if(this.collider)
      {
        if (this.collider instanceof OrientedBoundingBox) {
...
defineLazyP5Property('AABB', boundConstructorFactory(AABB));
  ...
  function OrientedBoundingBox(pInst, width, height, center, rotation) {
    ...
    this.collide = function (other) {
      ...
      var deltaOfCenters = p5.Vector.sub(other.center, _center);
      ...
      // separating axis.
      var candidateAxes = this.getPossibleAxesSeparatingThisFrom(other);
      var axis, deltaOfCentersOnAxis, distanceOfCentersOnAxis;
      for (var i = 0; i < candidateAxes.length; i++) {
        axis = p5.Vector.fromAngle(candidateAxes[i]);
        ...
        var overlap = this._getRadiusOnAxis(axis) + other._getRadiusOnAxis(axis) - distanceOfCentersOnAxis;
        if (overlap <= 0) {
