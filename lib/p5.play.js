...
defineLazyP5Property('AABB', boundConstructorFactory(AABB));
  ...
  function OBB(pInst, width, height, center, rotation) {
    // These are 'private' to OBB, but may be accessed by other instances of OBB.
    this._width = width;
    this._height = height;
    this._center = center;
    ...
    this.draw = function () {
      ...
      pInst.translate(this._center.x, this._center.y);
      if (pInst._angleMode === pInst.RADIANS) {
        pInst.rotate(radians(this._rotation));
      } else {
        pInst.rotate(this._rotation);
      }
      pInst.noFill();
      pInst.stroke('pink');
      pInst.strokeWeight(3);
      pInst.rect(0, 0, this._width, this._height);
      pInst.pop();
    };

    // Compute the smallest vector that would project this out of another collider.
    this.collide = function (other) {
      var differenceOfCenters = p5.Vector.sub(other._center, this._center);
      ...
      separatingAxes.push(this._rotation);
      separatingAxes.push(this._rotation + Math.PI / 2);

      // Push other shape's axes
      // TODO: extract to helper so we can generalize
      separatingAxes.push(other._rotation);
      separatingAxes.push(other._rotation + Math.PI / 2);
      ...
      for (var i = 0; i < separatingAxes.length; i++) {
        ...
        var thisHalfWidthVector = p5.Vector.fromAngle(this._rotation).mult(this._width / 2);
        pInst.LOGGING && console.log('thisHalfWidthVector: ' + thisHalfWidthVector);
        var thisHalfHeightVector = p5.Vector.fromAngle(this._rotation + Math.PI / 2).mult(this._height / 2);
        ...
        var otherHalfWidthVector = p5.Vector.fromAngle(other._rotation).mult(other._width / 2);
        var otherHalfHeightVector = p5.Vector.fromAngle(other._rotation + Math.PI / 2).mult(other._height / 2);
