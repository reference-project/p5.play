...
function Sprite(pInst, _x, _y, _w, _h) {
  ...
  */
  this.setCollider = function(type, offsetX, offsetY, width, height) {
    if (!(type === 'rectangle' || type === 'circle' || type === 'obb')) {
...
defineLazyP5Property('AABB', boundConstructorFactory(AABB));
  ...
  /**
   * An [O]riented [B]ounding [B]ox collision shape, used to detect overlap and
  ...
  function BoundingCircle(pInst, radius, center) {
    // These are 'private' to this BoundingCircle
    var _radius = radius;
    var _center = center;
    ...
    this.updateFromSprite = function (sprite) {
      ...
      // TODO: Let this collider store offsets, applied here.
      _radius = Math.floor(abs(sprite.scale) * Math.max(sprite.width, sprite.height) / 2);
      _center = sprite.position
    };
    ...
    this.draw = function () {
      ...
      pInst.stroke(0, 255, 0);
      pInst.rectMode(CENTER);
      pInst.ellipse(_center.x, _center.y, _radius*2, _radius*2);
      pInst.pop();
    };
