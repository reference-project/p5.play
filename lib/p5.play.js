...
function Sprite(pInst, _x, _y, _w, _h) {
  ...
  */
  this.update = function() {
    ...
    if(!this.removed)
    {
      ...
      if (this.collider instanceof CollisionShape) {
  ...
  */
  this.setCollider = function(type, offsetX, offsetY, width, height) {
    if (!(type === 'rectangle' || type === 'circle' || type === 'obb' || type === 'bc' || type === 'aabb')) {
      throw new TypeError('setCollider expects the first argument to be either "circle" or "rectangle" or "obb" or "aabb"');
    ...
    if (type === 'rectangle' && arguments.length === 1) {
      ...
      this.collider = new AxisAlignedBoundingBox(this.width, this.height, this.position);
    } else if (type === 'obb') {
      this.collider = new OrientedBoundingBox(this.width, this.height, this.position, radians(this.rotation));
    } else if (type === 'bc') {
      this.collider = new BoundingCircle(Math.floor(Math.max(this.width, this.height) / 2), this.position);
    }
        ...
        if (this.collider === undefined || other.collider === undefined) {
          ...
          if (displacement.x === 0 && displacement.y === 0) {
            ...
            if (type === 'collide' || type === 'displace' || type === 'bounce') {
              ...
              if (type === 'bounce') {
                var dx1, dx2, magnitude, totalMass, newVel1, newVel2;
                var m1 = 0, m2 = 0;
                if (this.collider instanceof CollisionShape && other.collider instanceof CollisionShape) {
                  // New collision system: Use the displacement direction to determine velocity change
                  dx1 = displacement.copy();
                  dx2 = p5.Vector.mult(dx1, -1);
                  magnitude = dx1.magSq();
                  totalMass = this.mass + other.mass;
                  if (this.immovable) {
                    m2 = 2;
                  ...
                  dx1 = p5.Vector.sub(this.position, other.position);
                  dx2 = p5.Vector.sub(other.position, this.position);
                  magnitude = dx1.magSq();
                  totalMass = this.mass + other.mass;
                  if (this.immovable) {
                    m2 = 2;
 ...
 * @param {p5.Vector} center
 */
function BoundingCircle(radius, center) {
...
BoundingCircle.prototype = Object.create(CollisionShape.prototype);
p5.BoundingCircle = BoundingCircle;
 ...
 * @param {Sprite} sprite
 */
BoundingCircle.prototype.updateFromSprite = function(sprite) {
 ...
 * @param {p5} sketch instance to use for drawing
 */
BoundingCircle.prototype.draw = function(sketch) {
  sketch.push();
  sketch.noFill();
 ...
 * @return {Array.<p5.Vector>}
 */
BoundingCircle.prototype._getCandidateAxes = function(other) {
  ...
  } else if (other instanceof OrientedBoundingBox || other instanceof AxisAlignedBoundingBox) {
 ...
 * @return {number}
 */
BoundingCircle.prototype._getRadiusOnAxis = function() {
