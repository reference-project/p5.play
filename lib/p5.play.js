...
function Sprite(pInst, _x, _y, _w, _h) {
  ...
  */
  this.update = function() {
    ...
    if(!this.removed)
    {
      ...
      if (this.collider) {
        this.collider.updateFromSprite(pInst, this);
      }
  ...
  this.mouseUpdate = function() {
    ...
    if(this.collider) {
      ...
      this.mouseIsOver = this.collider.overlap(new p5.CollisionPoint(mousePosition));
  ...
  */
  this.setCollider = function(type, offsetX, offsetY, width, height, rotation) {
    ...
    if (_type === 'point') {
      this.collider = p5.CollisionPoint.createFromSprite(pInst, this, offset);
    } else if (_type === 'circle') {
      this.collider = p5.CollisionCircle.createFromSprite(pInst, this, offset, width);
    } else if (_type === 'aabb') {
      this.collider = p5.AxisAlignedBoundingBox.createFromSprite(pInst, this, offset, width, height);
    } else if (_type === 'obb') {
      this.collider = p5.OrientedBoundingBox.createFromSprite(pInst, this, offset, width, height, rotation);
    }
  ...
  /**
   * Returns the axis-aligned bounding box of the sprite, based on the dimensions
   * of its current animation.
   * @method getBoundingBox
   * @return {p5.AxisAlignedBoundingBox}
   */
  this.getBoundingBox = function() {
    ...
    return new p5.AxisAlignedBoundingBox(this.position, w, h);
  };
  ...
  */
  this.overlapPoint = function(pointX, pointY) {
    ...
    if(this.collider) {
      var point = new p5.CollisionPoint(new p5.Vector(pointX, pointY));
 ...
 */
p5.CollisionShape = function(center) {
  /**
  ...
  this._localTransform = new p5.Transform2D()
    .translate(center || new p5.Vector());

  /**
   * The center of the collision shape in world-space.
   * Note - this duplicates the parent transform's resolved translation,
   * but it's held separate because it simplifies a lot.
   * @property _center
   * @private
   * @type {p5.Vector}
   */
 ...
 * the given sprite then recompute cached values if necessary.
 * @method updateFromSprite
 * @param {p5} sketch
 * @param {Sprite} sprite
 */
p5.CollisionShape.prototype.updateFromSprite = function(sketch, sprite) {
  this._parentTransform = new p5.Transform2D()
    .scale(sprite.scale) // TODO: ScaleX and ScaleY?
    .rotate(radians(sprite.rotation))
 ...
 * of the shape's transforms changes.  The base CollisionShape (and CollisionPoint)
...
p5.CollisionShape.prototype._onTransformChanged = function() {
  var compositeTransform = p5.Transform2D.mult(this._localTransform, this._parentTransform);

  // Reset and modify _center in-place so we don't need to create a new one.
  this._center.set(0, 0).transform(compositeTransform);
  this._rotation = compositeTransform.getRotation();
};
...
p5.CollisionShape.prototype.collide = function(other) {
  ...
  for (var i = 0; i < candidateAxes.length; i++) {
    ...
    //console.log('  deltaOfCentersOnAxis: ' + deltaOfCentersOnAxis);
 ...
 * @class p5.CollisionPoint
 * @constructor
 * @extends p5.CollisionShape
 * @param {p5.Vector} center
 */
p5.CollisionPoint = function(center) {
  p5.CollisionShape.call(this, center);
};
p5.CollisionPoint.prototype = Object.create(p5.CollisionShape.prototype);
 ...
 */
p5.CollisionPoint.prototype.draw = function(sketch) {
  sketch.push();
...
/**
 * Construct a new CollisionPoint with given offset for the given sprite.
 * @method createFromSprite
 * @static
 * @param {p5} sketch
 * @param {Sprite} sprite
 * @param {p5.Vector} [offset] from the sprite's center
 * @return {p5.CollisionPoint}
 */
p5.CollisionPoint.createFromSprite = function(sketch, sprite, offset) {
  // Create the collision shape at the transformed offset
  var shape = new p5.CollisionPoint();
  shape._localTransform = new p5.Transform2D().translate(offset);
  shape.updateFromSprite(sketch, sprite);
  return shape;
};
 ...
 * @class p5.CollisionCircle
 * @constructor
 * @extends p5.CollisionShape
 * @param {p5.Vector} center
 * @param {number} radius
 */
p5.CollisionCircle = function(center, radius) {
  p5.CollisionShape.call(this, center);
...
p5.CollisionCircle.prototype = Object.create(p5.CollisionShape.prototype);

/**
 * Construct a new CollisionCircle with given offset for the given sprite.
 * @method createFromSprite
 * @static
 * @param {p5} sketch
 * @param {Sprite} sprite
 * @param {p5.Vector} [offset] from the sprite's center
 * @param {number} [radius]
 * @return {p5.CollisionCircle}
 */
p5.CollisionCircle.createFromSprite = function(sketch, sprite, offset, radius) {
  var inferredRadius = Math.floor(Math.max(sprite.width, sprite.height)/2);
  var scale = radius ? (radius/inferredRadius) : 1;

  // Create the collision shape at the transformed offset
  var shape = new p5.CollisionCircle();
  shape._localTransform = new p5.Transform2D()
    .scale(scale)
    .translate(offset);
  shape.updateFromSprite(sketch, sprite);
  return shape;
 ...
 * @param {Sprite} sprite
 */
p5.CollisionCircle.prototype.updateFromSprite = function(sketch, sprite) {
  p5.CollisionShape.prototype.updateFromSprite.call(this, sketch, sprite);
  this.radius = new p5.Vector(Math.floor(Math.max(sprite.width, sprite.height)/2))
 ...
 * @param {p5} sketch instance to use for drawing
 */
p5.CollisionCircle.prototype.draw = function(sketch) {
  sketch.push();
  sketch.noFill();
 ...
 * @return {Array.<p5.Vector>}
 */
p5.CollisionCircle.prototype._getCandidateAxes = function(other) {
  ...
  if (other instanceof p5.OrientedBoundingBox || other instanceof p5.AxisAlignedBoundingBox) {
 ...
 * @return {number}
 */
p5.CollisionCircle.prototype._getRadiusOnAxis = function() {
 ...
 *
 * @class p5.AxisAlignedBoundingBox
 * @constructor
 * @extends p5.CollisionShape
 * @param {p5.Vector} center
 * @param {number} width
 * @param {number} height
 */
p5.AxisAlignedBoundingBox = function(center, width, height) {
  p5.CollisionShape.call(this, center);
...
p5.AxisAlignedBoundingBox.prototype = Object.create(p5.CollisionShape.prototype);
 ...
 * @final
 */
p5.AxisAlignedBoundingBox.WORLD_AXES = [
  new p5.Vector(1, 0),
  new p5.Vector(0, 1)
];
 ...
 * @return {Array.<p5.Vector>}
 */
p5.AxisAlignedBoundingBox.prototype._computeHalfDiagonals = function() {
  // For an axis-aligned rectangle this is the halfwidth vector x halfheight vector.
  var halfWidth = p5.Vector.mult(p5.AxisAlignedBoundingBox.WORLD_AXES[0], this._width / 2);
  var halfHeight = p5.Vector.mult(p5.AxisAlignedBoundingBox.WORLD_AXES[1], this._height / 2);
  return [
...
/**
 * Construct a new AxisAlignedBoundingBox with given offset for the given sprite.
 * @method createFromSprite
 * @static
 * @param {p5} sketch
 * @param {Sprite} sprite
 ...
 * @return {p5.CollisionCircle}
 */
p5.AxisAlignedBoundingBox.createFromSprite = function(sketch, sprite, offset) {
  // Create the collision shape at the transformed offset
  var shape = new p5.AxisAlignedBoundingBox();
  shape._localTransform = new p5.Transform2D()
    .translate(offset);
  shape.updateFromSprite(sketch, sprite);
  return shape;
 ...
 * @param {Sprite} sprite
 */
p5.AxisAlignedBoundingBox.prototype.updateFromSprite = function(sketch, sprite) {
  p5.CollisionShape.prototype.updateFromSprite.call(this, sketch, sprite);

  // Update internal _width and _height -without- transforms, because they
 ...
 * @param {Sprite} sprite
 */
p5.AxisAlignedBoundingBox.prototype.updateFromSprite = function(sprite) {
 ...
 */
p5.AxisAlignedBoundingBox.prototype.draw = function(sketch) {
  sketch.push();
  sketch.rectMode(sketch.CENTER);
  sketch.translate(this.center.x, this.center.y);
  sketch.noFill();
 ...
 * @return {Array.<p5.Vector>}
 */
p5.AxisAlignedBoundingBox.prototype._getCandidateAxes = function() {
  return p5.AxisAlignedBoundingBox.WORLD_AXES;
};
 ...
 * @return {number}
 */
p5.AxisAlignedBoundingBox.prototype._getRadiusOnAxis = function(axis) {
 ...
 */
p5.OrientedBoundingBox = function(center, width, height, rotation) {
  p5.CollisionShape.call(this, center);

  /* @private {number} Unscaled unrotated box width. */
  this._width = width;
  /* @private {number} Unscaled unrotated box height. */
  this._height = height;
  ...
  // Initialize the local transform with the box rotation
  this._localTransform = new p5.Transform2D()
    .rotate(rotation || 0)
    .translate(center || new p5.Vector());
...
p5.OrientedBoundingBox.prototype = Object.create(p5.CollisionShape.prototype);
 ...
 * @protecte
 */
p5.OrientedBoundingBox.prototype._onTransformChanged = function() {
  p5.CollisionShape.prototype._onTransformChanged.call(this);

  // Transform each vertex by the local and global matrices
  // then use their differences to determine width, height, and halfDiagonalso
  ...
  // TODO: Could split these update operations and do less in certain cases?
  this._halfDiagonals = [
 ...
 * @return {p5.CollisionCircle}
 */
p5.OrientedBoundingBox.createFromSprite = function(sketch, sprite, offset, width, height, rotation) {
  // Create the collision shape at the transformed offset
  var shape = new p5.OrientedBoundingBox();
  shape._width = width !== undefined ? width : sprite.width;
  shape._height = height !== undefined ? height : sprite.height;
  shape._rotation = rotation || 0;
  shape._localTransform = new p5.Transform2D()
    .rotate(shape._rotation)
    .translate(offset);
  shape.updateFromSprite(sketch, sprite);
  return shape;
};
 ...
 * @param {Sprite} sprite
 */
p5.OrientedBoundingBox.prototype.updateFromSprite = function(sketch, sprite) {
  // Transform center
  p5.CollisionShape.prototype.updateFromSprite.call(this, sketch, sprite);
  this._onTransformChanged();
  // TODO: Handle sprite width/height changing here?
};
 ...
 * @param {p5} sketch - p5 instance to use for drawing
 */
p5.OrientedBoundingBox.prototype.draw = function(sketch) {
  sketch.push();
  sketch.rectMode(sketch.CENTER);
  sketch.translate(this.center.x, this.center.y);
 ...
 * @return {Array.<p5.Vector>}
 */
p5.OrientedBoundingBox.prototype._getCandidateAxes = function() {
 ...
 * @return {number}
 */
p5.OrientedBoundingBox.prototype._getRadiusOnAxis = function(axis) {
  // How to project a rect onto an axis:
  // Project the center-corner vectors for two adjacent corners (cached here)
  // onto the axis.  The larger magnitude of the two is your projection's radius.
  return Math.max(
...
p5.prototype._warn = function(message) {
  ...
  p5.Transform2D = function(source) {
    ...
    this.splice.apply(this, [0, 0].concat(source));
  };
  ...
  p5.Transform2D.prototype.log = function () {
    console.log(
