  ...
  */
  this.update = function() {
    ...
    if(!this.removed)
    {
      ...
      if (this.collider) {
        this.collider.setParentTransform(this);
      }
    ...
    for(var i=0; i<others.length; i++) {
      if (this !== others[i] && !this.removed) //you can check collisions within the same group but not on itself
      {
        ...
        if (!this.collider || !other.collider) {
          ...
          this.collider.setParentTransform(this);
          other.collider.setParentTransform(other);
...
p5.CollisionCircle.createFromSprite = function(sprite, offset, radius) {
  var shape = new p5.CollisionCircle(
    offset,
    typeof radius === 'number' ? radius : Math.max(sprite.width, sprite.height)/2
  );
  shape.setParentTransform(sprite);
  return shape;
};
...
p5.AxisAlignedBoundingBox.createFromSprite = function(sprite, offset, width, height) {
  var box = new p5.AxisAlignedBoundingBox(
    offset,
    typeof width === 'number' ? width : sprite.width,
    typeof height === 'number' ? height : sprite.height
  );
  box.setParentTransform(sprite);
  return box;
};
...
p5.AxisAlignedBoundingBox.prototype._computeHalfDiagonals = function() {
  ...
  var halfWidth = Math.max(
    transformedDiagonals[0].x,
    transformedDiagonals[1].x
  );
  var halfHeight = Math.max(
    transformedDiagonals[1].y,
    transformedDiagonals[2].y
  );
...
p5.OrientedBoundingBox.prototype._onTransformChanged = function() {
  ...
  this.transformedVertices_ = transformedVertices;
...
p5.OrientedBoundingBox.prototype.draw = function(sketch) {
  ...
  // Draw half-diagonals
  //sketch.push();
  //var p1 = p5.Vector.add(this.center, this._halfDiagonals[0]);
  //var p2 = p5.Vector.add(this.center, this._halfDiagonals[1]);
  //sketch.strokeWeight(2);
  //sketch.stroke('red');
  //sketch.line(this.center.x, this.center.y, p1.x, p1.y);
  //sketch.rectMode(sketch.CENTER);
  //sketch.rect(p1.x, p1.y, 5, 5);
  //sketch.line(this.center.x, this.center.y, p2.x, p2.y);
  //sketch.rectMode(sketch.CENTER);
  //sketch.rect(p2.x, p2.y, 5, 5);
  //sketch.pop();

  // Draw axes
  //sketch.push();
  //p1 = p5.Vector.add(this.center, p5.Vector.mult(this._potentialAxes[0], 0.5));
  //p2 = p5.Vector.add(this.center, p5.Vector.mult(this._potentialAxes[1], 0.5));
  //sketch.strokeWeight(1);
  //sketch.stroke('blue');
  //sketch.line(this.center.x, this.center.y, p1.x, p1.y);
  //sketch.rectMode(sketch.CENTER);
